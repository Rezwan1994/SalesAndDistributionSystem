...
namespace SalesAndDistributionSystem.Areas.SalesAndDistribution.Controllers
{
    ...
    public class CustomerController : Controller
    {
        ...
        [HttpGet]
        public async Task<string> LoadActiveCustomerData(int COMPANY_ID)
        {
            ...
            else
            {
                try
                {
                    if (model.CUSTOMER_ID == 0)
                    {
                        ...
                        model.OPENING_DATE = DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss");
                        model.COMPANY_ID = model.COMPANY_ID == 0 ? Convert.ToInt32(User.Claims.FirstOrDefault(x => x.Type == ClaimsType.CompanyId).Value) : model.COMPANY_ID;
                        model.CLOSING_DATE = DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss");
                    }
                    else
                    {
